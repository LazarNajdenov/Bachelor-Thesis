...
%     Draw legend bar
    line([legLim(kk),legLim(kk+1)],[legY,legY],'lineWidth',25,'color',edgeMap(kk,:));
  end  % for kk
  
%   % Draw Legend Label
%   text([legLim(1),legLim(end)],[legY-0.5*yStep,legY-0.5*yStep], ...
%     {num2str(minEWt),num2str(maxEWt)},'HorizontalAlignment','center','color',[0,0,0],'Fontsize',16);
  % Draw Legend Label for Ecoli
%   text([legLim(1),legLim(end)],[legY-0.5*yStep,legY-0.5*yStep], ...
%     {0.004,0.9},'HorizontalAlignment','center','color',[0,0,0],'Fontsize',64,'interpreter','latex');
else
    nSegment=length(ii);
